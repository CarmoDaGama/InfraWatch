...
const ROUTES = [
  { id: 'dashboard', icon: '📊', label: 'sidebar.dashboard' },
  { id: 'devices', icon: '🖥️', label: 'sidebar.devices' },
  { id: 'integrations', icon: '🔗', label: 'sidebar.integrations' },
  { id: 'users', icon: '👥', label: 'sidebar.users', adminOnly: true },
]
    ...
    <aside className="w-20 sm:w-72 p-2 sm:p-4 sm:pr-3 h-screen">
      ...
      <nav className="flex-1 overflow-y-auto p-2 sm:p-4 space-y-2">
        {ROUTES.filter((route) => !route.adminOnly || isAdmin).map((route) => (
          ...
          >
            <span className="text-lg w-7 h-7 rounded-lg bg-black/20 flex items-center justify-center">{route.icon}</span>
